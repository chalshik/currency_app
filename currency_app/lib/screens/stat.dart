...
class _CurrencyStatsPageState extends State<CurrencyStatsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<CurrencyStat>>(
        future: _currencyStats, // Асинхронно загружаем статистику валют
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator()); // Загрузка
          } else if (snapshot.hasError) {
            return Center(child: Text('Error: ${snapshot.error}')); // Ошибка
                  ...
                  FutureBuilder<List<Map<String, dynamic>>>(
                    ...
                    builder: (context, currenciesSnapshot) {
                      if (currenciesSnapshot.connectionState ==
                        ...
                        return const Center(child: Text('No currencies data.'));
                      }
                    },
                  ),
                ],
